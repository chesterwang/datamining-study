...
object ALSExample {
  ...
  object Rating {//tongpeng:case class Rating和object Rating
  ...
  def main(args: Array[String]) {
    ...
    val ratings = sc.textFile("data/mllib/als/sample_movielens_ratings.txt")
      ...
      .toDF()//tongpeng:rdd -> df(自动解析为对应字段)
...
//      .setRatingCol("rating")
    ...
    val predictions = model.transform(test)
      .withColumn("rating", col("rating").cast(DoubleType))//tongpeng:字段类型转换 col属于functions
