...
object CrossValidatorExample {
  ...
  def main(args: Array[String]) {
    ...
    val tokenizer = new Tokenizer()
      .setInputCol("text")//tongpeng:自动根据LabeledDocument 这个case class进行解析
      .setOutputCol("words")
    ...
    println("--------")
    println(new BinaryClassificationEvaluator().explainParams())
    ...
    val cvModel = crossval.fit(training.toDF())//tongpeng:自动根据LabeledDocument 这个case class进行解析
    ...
    // Make predictions on test documents. cvModel uses the best model found (lrModel).tongpeng:使用最好的模型
