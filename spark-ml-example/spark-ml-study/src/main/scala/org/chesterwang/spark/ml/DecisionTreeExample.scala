...
object DecisionTreeExample {
    ...
    format match {
      ...
      case "libsvm" => expectedNumFeatures match {
        case Some(numFeatures) => sqlContext.read.option("numFeatures", numFeatures.toString)
          .format("libsvm").load(path)//tongpeng:选项
  ...
  def evaluateClassificationModel(
    ...
    val numClasses = Attribute.fromStructField(fullPredictions.schema(labelColName)) match {
      case binAttr: BinaryAttribute => 2
      case nomAttr: NominalAttribute => nomAttr.getNumValues
      case _: NumericAttribute | UnresolvedAttribute => throw new RuntimeException(
        "Unknown failure when indexing labels for classification.")
    }
    //tongpeng:predictions.zip(labels)
