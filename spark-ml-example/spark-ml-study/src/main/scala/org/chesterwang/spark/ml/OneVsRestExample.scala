...
import org.chesterwang.spark.ml.util.MetadataUtils
...
object OneVsRestExample {
  ...
  private def run(params: Params) {
    ...
    val Array(train, test) = data.map(_.cache())//tongpeng:分拆开来
    ...
    params.regParam.foreach(classifier.setRegParam)//tongpeng:foreach　多次set,最后导致每一个classifier都尝试一次
    params.elasticNetParam.foreach(classifier.setElasticNetParam)

    // instantiate the One Vs Rest Classifier.

    val ovr = new OneVsRest()//tongpeng:k类 k个模型
    ovr.setClassifier(classifier)

    // train the multiclass model.
    val (trainingDuration, ovrModel) = time(ovr.fit(train))//tongpeng:计时
    ...
    val confusionMatrix = metrics.confusionMatrix//tongpeng:混淆矩阵
